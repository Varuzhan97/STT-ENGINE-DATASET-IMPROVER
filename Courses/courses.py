...
def get_answer(stt, vad_audio, correct_answer, numbers_speech, repeat_speech, again_repeat_speech, excellent_speech, current_command_id, collection_folder, validation):
    if validation:
        while True:
            file_name = str()
            file_size = str()
            answer, file_name, file_size = stt.listen_audio(vad_audio, save_wav = True, save_wav_path = collection_folder)
            print("Microphone record: ", answer)
            if answer == '':
...
def start_course(courses_tts_folder, courses_data_folder, vad_audio, stt, model_path, config_file_data, config_file, collection_folder, validation):
    ...
    for i in range(current_command_id - len(numbers_keys_list), len(conversation_keys_list)):
        ...
        current_command_id, file_name, file_size = get_answer(stt, vad_audio, correct_answer, current_command_speech, repeat_speech, again_repeat_speech, excellent_speech, current_command_id, collection_folder, int(validation))
        #Check for audio length and update csv
        if utils.check_audio(correct_answer, file_name, file_size):
            #Update collected data in csv
            utils.write_to_csv(file_name, file_size, correct_answer, collection_folder)
